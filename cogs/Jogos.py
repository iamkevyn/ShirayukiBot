...
def ensure_dir_exists(file_path):
    ...
    if not os.path.exists(directory):
        ...
        print(f"[{datetime.now().strftime(\"%Y-%m-%d %H:%M:%S\")}] [INFO] Diretório criado: {directory}")
...
def load_json_data(file_path, default_data):
    ...
    if not os.path.exists(file_path):
        print(f"[{datetime.now().strftime(\"%Y-%m-%d %H:%M:%S\")}] [AVISO] Arquivo {os.path.basename(file_path)} não encontrado. Criando com dados padrão.")
    ...
    try:
        ...
        print(f"[{datetime.now().strftime(\"%Y-%m-%d %H:%M:%S\")}] [ERRO] Falha ao carregar {os.path.basename(file_path)}, usando dados padrão.")
        return default_data
...
class Jogos(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.hangman_words = load_json_data(HANGMAN_WORDS_FILE, DEFAULT_HANGMAN        print(f"[{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}] Cog Jogos carregada.")     print(f"[Jogos] {len(self.quiz_questions)} perguntas de quiz carregadas.")
